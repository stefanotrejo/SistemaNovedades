...
public partial class PerfilConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Perfil - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["PerfilConsulta.PageIndex"] == null)
                {
                    Session.Add("PerfilConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["PerfilConsulta.PageIndex"]);
                }
                #endregion

                #region Variables de sesion para filtros
//if (Session["PerfilConsulta.Nombre"] != null) { Perfil.Text = Session["PerfilConsulta.Nombre"].ToString(); } else { Session.Add("PerfilConsulta.Nombre", Nombre.Text.Trim()); }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("PerfilRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    {
        dt = new DataTable();
        dt = ocnPerfil.ObtenerTodoBuscarxNombre(Nombre.Text.Trim());
        string ArchivoNombre = "PerfilConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";

...
public partial class AyudaConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Ayuda - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["AyudaConsulta.PageIndex"] == null)
                {
                    Session.Add("AyudaConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["AyudaConsulta.PageIndex"]);
                }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("AyudaRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    {
        dt = new DataTable();
        dt = ocnAyuda.ObtenerTodo();
        string ArchivoNombre = "AyudaConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["AyudaConsulta.PageIndex"] = PageIndex;
    ...
    protected void Grilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
	    try
	    {
		    if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
		    {
			    ...
			    {
				    //ocnAyuda.Eliminar(Convert.ToInt32(Id));
				    this.GrillaCargar(this.Grilla.PageIndex);
			    }

			    if (e.CommandName == "Copiar")
			    {
				    ocnAyuda = new LiquidacionSueldos.Negocio.Ayuda(Convert.ToInt32(Id));
				    //ocnAyuda.Copiar();
				    this.GrillaCargar(this.Grilla.PageIndex);
			    }
			    ...
			    {
				    Response.Redirect("AyudaRegistracion.aspx?Id=" + Id + "&Ver=1", false);
			    }			
		    }
	    }
	    catch (Exception oError)
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["AyudaConsulta.PageIndex"] != null)
            {
                Session["AyudaConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("AyudaConsulta.PageIndex", e.NewPageIndex);
            }

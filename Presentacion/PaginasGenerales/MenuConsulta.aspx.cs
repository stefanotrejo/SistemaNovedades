...
public partial class MenuConsulta : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!Page.IsPostBack)
            {
                this.Master.TituloDelFormulario = " Menu - Consulta";

                //if (this.Session["_Autenticado"] == null) Response.Redirect("Login.aspx", true);

                #region PageIndex
                int PageIndex = 0;
                if (this.Session["MenuConsulta.PageIndex"] == null)
                {
                    Session.Add("MenuConsulta.PageIndex", 0);
                }
                else
                {
                    PageIndex = Convert.ToInt32(Session["MenuConsulta.PageIndex"]);
                }
                #endregion

                #region Variables de sesion para filtros
//if (Session["MenuConsulta.Nombre"] != null) { Menu.Text = Session["MenuConsulta.Nombre"].ToString(); } else { Session.Add("MenuConsulta.Nombre", Nombre.Text.Trim()); }
                #endregion
...
"</div>";
    ...
    protected void btnNuevo_Click(object sender, EventArgs e)
    {
	    try
	    {
            Response.Redirect("MenuRegistracion.aspx?Id=0", false);
	    }
	    catch (Exception oError)
	    {
            lblMensajeError.Text = @"<div class=""alert alert-danger alert-dismissable"">
...
"</div>";
    ...
    {
        dt = new DataTable();
        dt = ocnMenu.ObtenerTodoBuscarxNombre(Nombre.Text.Trim());
        string ArchivoNombre = "MenuConsulta_" + DateTime.Now.Day.ToString() + "-" + DateTime.Now.Month.ToString() + "-" + DateTime.Now.Year.ToString() + ".xls";
    ...
    private void GrillaCargar(int PageIndex)
    {
        try
        {
            Session["MenuConsulta.PageIndex"] = PageIndex;
    ...
    protected void Grilla_RowCommand(object sender, GridViewCommandEventArgs e)
    {
	    try
	    {
		    if (e.CommandName != "Sort" && e.CommandName != "Page" && e.CommandName != "")
		    {
			    ...
			    {
				    Response.Redirect("MenuRegistracion.aspx?Id=" + Id + "&Ver=1", false);
			    }			
		    }
	    }
	    catch (Exception oError)
...
"</div>";
    ...
    protected void Grilla_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
	    try
	    {
            if (Session["MenuConsulta.PageIndex"] != null)
            {
                Session["MenuConsulta.PageIndex"] = e.NewPageIndex;
            }
            else
            {
                Session.Add("MenuConsulta.PageIndex", e.NewPageIndex);
            }

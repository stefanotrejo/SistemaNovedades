...
SELECT 	@mesanioPruebasAge = '01/24'
...
DECLARE @complementaria int
SELECT @complementaria = 1
...
--UPDATE agentes_extdoc_diferencia_saldo_24agosto set monto_destino_rem_descontado = 0.00
--UPDATE agentes_extdoc_diferencia_saldo_24agosto set monto_origen_rem_descontar = 0.00
...
UPDATE 
	agentes_extdoc_diferencia_saldo_24agosto
SET 
	monto_destino_rem_descontado = '0.00',
	monto_destino_norem_descontado = '0.00'
...
SET 
	monto_destino_rem_descontado =
								(CASE WHEN IMP_521 >= convert(decimal (18,2), ISNULL(t2.monto_origen_rem_descontar,0))
									THEN t2.monto_origen_rem_descontar
									ELSE ISNULL(Convert(varchar, IMP_521), 0.00) 
									END),
	monto_destino_norem_descontado =
									...
									ELSE ISNULL(Convert(varchar,IMP_522), 0.00) 
									END),
	fecha_descuento = GETDATE()	
FROM
	#agentes_filtrados t1
	INNER JOIN agentes_extdoc_diferencia_saldo_24agosto t2
	ON T1.numeroControl = t2.NroCOntrol
...
UPDATE 
	agentes_extdoc_diferencia_saldo_24agosto
SET
	saldo_rem = ISNULL(convert(decimal (18,2),t2.monto_origen_rem_descontar),0) - ISNULL(convert(decimal (18,2),monto_destino_rem_descontado),0),
	saldo_norem = ISNULL(convert(decimal (18,2),t2.monto_origen_norem_descontar),0) - ISNULL(convert(decimal (18,2),monto_destino_norem_descontado),0)
FROM
	agentes_extdoc_diferencia_saldo_24agosto t2 	
...
select * from agentes_extdoc_diferencia_saldo_24agosto t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl -- 90 encuentra controles

-- PERSONAS QUE ESTAN EN LA LIQUIDACION PERO NO TIENEN NO_REM PARA APLICAR DESCUENTO - 7
select * from agentes_extdoc_diferencia_saldo_24agosto t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl
where not exists (
		select * from agentes_extdoc_diferencia_saldo_24agosto t3
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 
and t3.NroCOntrol = t2.numeroControl
)


-- TOTAL DESCONTADOS - 51
select * from agentes_extdoc_diferencia_saldo_24agosto
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 

-- PARCIAL o NULO -  xx / 17
select * from agentes_extdoc_diferencia_saldo_24agosto
...
from agentes_extdoc_diferencia_saldo_24agosto
where
--	 CONVERT(Numeric (18,2),saldo_norem) = 0 -- LES DESCONTÓ TODO
--AND 
CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0

-- NO LOS ENCONTRÓ O NO TENIAN PARA DESCONTAR XXX / 8
select * from agentes_extdoc_diferencia_saldo_24agosto
where CONVERT(numeric (18,2), monto_destino_norem_descontado) <= 0

-- AGENTES A DESCONTAR ORIGINALMENTE -- XXX / 340
select * from agentes_extdoc_diferencia_saldo_24agosto  t1
where 	
...
select * from agentes_extdoc_diferencia_saldo_24agosto where activoNoRem = 1
...
-- TOTAL DESCUENTOS GENERAL -> xxx / $140.228.15
SELECT  				
...
FROM  
	#agentes_filtrados t1		
	LEFT JOIN agentes_extdoc_diferencia_saldo_24agosto t3
	ON t1.numeroControl = t3.NroCOntrol



--SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_24agosto
--where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
----------------------->>>>> FIN GENERACION TXT PARA EL BANCO V2 <<<<<-----------------------
 ...
 -- DUPLICADOS -- 
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_24agosto 
...
and monto_destino_norem_descontado != '0.00'
...
from agentes_extdoc_diferencia_saldo_24agosto t1

-- TOTAL DESCUENTOS GENERAL -> xxx / $280083.77
SELECT  				
	 SUM (convert(decimal (18,2), t3.monto_destino_norem_descontado))
	 --count(1)
AS Columna 
...
exec [ExtensionDocente.Guardar_En_Historico] '01', '24', 80, 0, 0

exec [ExtensionDocente.Eliminar_En_Historico] '01','24',80
...
exec [ExtensionDocente.Archivo_Ministerio] '01', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 80, 0,0,0

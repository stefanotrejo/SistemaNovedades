...
select * from #agentes_filtrados t1 inner join agentes_extdoc_diferencia_saldo_mayo t2 
...
update 
	#agentes_filtrados
	set IMP_521 = IMP_521 * 0.7 -- SE PAGA EL 70% POR CUESTIONES PRESUPUESTARIAS
from #agentes_filtrados t1 

update 
	#agentes_filtrados
	set IMP_522 = IMP_522 * 0.7 -- SE PAGA EL 70% POR CUESTIONES PRESUPUESTARIAS
from #agentes_filtrados t1 




--UPDATE agentes_extdoc_diferencia_saldo_mayo set monto_destino_rem_descontado = 0.00
--UPDATE agentes_extdoc_diferencia_saldo_mayo set monto_origen_rem_descontar = 0.00


--select * from agentes_extdoc_diferencia_saldo_mayo
...
UPDATE 
	agentes_extdoc_diferencia_saldo_mayo
SET 
	monto_destino_rem_descontado = '0',
	monto_destino_norem_descontado = '0'
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_saldo_mayo t2
	ON T1.numeroControl = t2.NroCOntrol
...
select * from agentes_extdoc_diferencia_saldo_mayo t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl -- 869 encuentra controles

-- PERSONAS QUE ESTAN EN LA LIQUIDACION PERO NO TIENEN NO_REM PARA APLICAR DESCUENTO - 10
select * from agentes_extdoc_diferencia_saldo_mayo t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl
where not exists (
		select * from agentes_extdoc_diferencia_saldo_mayo t3
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 
and t3.NroCOntrol = t2.numeroControl
)


-- TOTAL DESCONTADOS - 859
select * from agentes_extdoc_diferencia_saldo_mayo
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 

-- PARCIAL o NULO -  xx / 229
select * from agentes_extdoc_diferencia_saldo_mayo
...
from agentes_extdoc_diferencia_saldo_mayo
where
--	 CONVERT(Numeric (18,2),saldo_norem) = 0 -- LES DESCONTÓ TODO
--AND 
CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0

-- NO LOS ENCONTRÓ O NO TENIAN PARA DESCONTAR XXX / 13
select * from agentes_extdoc_diferencia_saldo_mayo
where CONVERT(numeric (18,2), monto_destino_norem_descontado) <= 0

-- AGENTES A DESCONTAR ORIGINALMENTE -- XXX / 872
select * from agentes_extdoc_diferencia_saldo_mayo  t1
where 	
...
-- TOTAL DESCUENTOS GENERAL -> xxx / $3.373.129.46 (70)
SELECT  				
...
FROM  
	#agentes_filtrados t1		
	LEFT JOIN agentes_extdoc_diferencia_saldo_mayo t3
	ON t1.numeroControl = t3.NroCOntrol



--SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_mayo
--where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
WHERE
 ...
 -- DUPLICADOS -- 
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_mayo 
...
from agentes_extdoc_diferencia_saldo_mayo t1

-- TOTAL DESCUENTOS GENERAL -> xxx / $3.373.129,46
SELECT  				
	 SUM (convert(decimal (18,2), t3.monto_destino_norem_descontado))
	 --count(1)
AS Columna 
...
SELECT COUNT(1)  from agentes_extension_docente_historico
where liq_id = 51
...
exec [ExtensionDocente.Guardar_En_Historico] '05', '24', 53, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '05', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 53, 0,0,0

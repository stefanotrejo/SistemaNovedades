...
select * from #agentes_filtrados t1 inner join agentes_extdoc_diferencia_saldo_julio t2 
...
--UPDATE agentes_extdoc_diferencia_saldo_julio set monto_destino_rem_descontado = 0.00
--UPDATE agentes_extdoc_diferencia_saldo_julio set monto_origen_rem_descontar = 0.00


--select * from agentes_extdoc_diferencia_saldo_julio
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_saldo_julio t2
	ON T1.numeroControl = t2.NroCOntrol
...
select * from agentes_extdoc_diferencia_saldo_julio t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl -- 90 encuentra controles

-- PERSONAS QUE ESTAN EN LA LIQUIDACION PERO NO TIENEN NO_REM PARA APLICAR DESCUENTO - 5
select * from agentes_extdoc_diferencia_saldo_julio t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl
where not exists (
		select * from agentes_extdoc_diferencia_saldo_julio t3
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 
and t3.NroCOntrol = t2.numeroControl
)


-- TOTAL DESCONTADOS - 85
select * from agentes_extdoc_diferencia_saldo_julio
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 

-- PARCIAL o NULO -  xx / 37
select * from agentes_extdoc_diferencia_saldo_julio
...
from agentes_extdoc_diferencia_saldo_julio
where
...
-- TOTAL DESCUENTOS GENERAL -> xxx / $280.083.77
SELECT  				
...
FROM  
	#agentes_filtrados t1		
	LEFT JOIN agentes_extdoc_diferencia_saldo_julio t3
	ON t1.numeroControl = t3.NroCOntrol



--SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_julio
--where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
WHERE
 ...
 -- DUPLICADOS -- 
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_julio 
...
from agentes_extdoc_diferencia_saldo_julio t1
...
exec [ExtensionDocente.Guardar_En_Historico] '07', '24', 61, 0, 0

...
/*
REEMPLAZAR:
@mesanioPruebasAge = '09/23'
agentes_extdoc_diferencia_0123
...
SELECT 	@mesanioPruebasAge = '09/23'
...
UPDATE agentes_extdoc_diferencia_0123 set monto_destino_rem_descontado = null
UPDATE agentes_extdoc_diferencia_0123 set monto_destino_norem_descontado = null
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_0123 t2
...
6645 - encuentra los controles 

-- PARCIAL 2449 / 2501
select * from agentes_extdoc_diferencia_0123
...
-- NO LOS ENCONTRÓ 418 / 417
select * from agentes_extdoc_diferencia_0123
where saldo_norem is null
AND CONVERT(numeric (18,2),monto_origen_norem_descontar) > 0 

-- AGENTES A DESCONTAR ORIGINALMENTE -- 5643 / 5643
select * from agentes_extdoc_diferencia_0123  t1
where 	
...
--	#agentes_filtrados t1
--INNER JOIN 
--	agentes_extdoc_diferencia_0123 t2
--ON  T1.numeroControl = t2.NroCOntrol
...
-- TOTAL DESCUENTOS GENERAL -> $18.495.806.48 / $18.219.577,14
SELECT  				
...
FROM  
	...
	LEFT JOIN agentes_extdoc_diferencia_0123 t3
	ON t1.numeroControl = t3.NroCOntrol
	 AND t3.id not in (2273)
...
FROM  
 ...
 select COUNT(1) from #agentes_filtrados
 select  NroCOntrol from agentes_extdoc_diferencia_0123 
...
select SUM (CONVERT(numeric (18,2), monto_destino_norem_descontado)) from agentes_extdoc_diferencia_0123
...
select * from agentes_extension_docente t1
where exists (
select * from NovedadesExtensionDocente t2
where liq_id = 33
and activo = 1

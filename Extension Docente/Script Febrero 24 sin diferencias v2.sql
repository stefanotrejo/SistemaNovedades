...
agentes_extdoc_diferencia_saldo_abril
...
SELECT 	@mesanioPruebasAge = '02/24'
...
select * from #agentes_filtrados t1 inner join agentes_extdoc_diferencia_saldo_abril t2 
...
--1701 -- encuentra los controles 

-- Casos que no encontró para descontar en la liquidacion
select * from agentes_extdoc_diferencia_saldo_abril t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl -- 1701 encuentra controles

-- PERSONAS QUE ESTAN EN LA LIQUIDACION PERO NO TIENEN NO_REM PARA APLICAR DESCUENTO
select * from agentes_extdoc_diferencia_saldo_abril t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl
where not exists (
		select * from agentes_extdoc_diferencia_saldo_abril t3
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 
and t3.NroCOntrol = t2.numeroControl
)


-- TOTAL DESCONTADOS
select * from agentes_extdoc_diferencia_saldo_abril
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 

-- PARCIAL o NULO -  xx / 872
select * from agentes_extdoc_diferencia_saldo_abril
...
-- $7.670.735.50
select sum (CONVERT(numeric (18,2), monto_destino_norem_descontado))
from agentes_extdoc_diferencia_saldo_abril
where
--	 CONVERT(Numeric (18,2),saldo_norem) = 0 -- LES DESCONTÓ TODO
--AND 
CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0

-- NO LOS ENCONTRÓ O NO TENIAN PARA DESCONTAR XXX / 191
select * from agentes_extdoc_diferencia_saldo_abril
where CONVERT(numeric (18,2), monto_destino_norem_descontado) <= 0

-- AGENTES A DESCONTAR ORIGINALMENTE -- XXX / 1862
select * from agentes_extdoc_diferencia_saldo_abril  t1
where 	
 CONVERT(numeric (18,2),monto_origen_norem_descontar) > 0

---- DESGLOSE DE LOS QUE NO ENCONTRÓ
---- 144
--select * from agentes_extdoc_diferencia_saldo_abril t1
...
inner join agentes_extdoc_diferencia_saldo_abril  t2
on t1.numeroControl = t2.NroCOntrol


-- TOTAL DESCUENTOS GENERAL -> xxx / $7.670.735.50
SELECT  				
...
FROM  
	#agentes_filtrados t1		
	LEFT JOIN agentes_extdoc_diferencia_saldo_abril t3
	ON t1.numeroControl = t3.NroCOntrol


select * from #agentes_filtrados
where numeroControl = '38759492'


select 
NroCOntrol,
	CONVERT(numeric (18) , CAST (dbo.RellenarTextoIzquierda(replace(cast(
			  (convert(decimal (18,2), t1.IMP_521) + (convert(decimal (18,2),t1.IMP_522))) -- total hab.
			- (
					 ISNULL(convert(decimal (18,2), null),'000000000') -- 601
					...
					+ ISNULL(convert(decimal (18,2), null),'000000000') -- 746
				) 
				 -- tot desc. 
			AS decimal(18,2)),'.',''),'0',9) AS VARCHAR(9))) as "liquido actual",  -- CALCULO DEL LIQUIDO,	
			CONVERT(numeric (18,0), t2.total_liquido) as liquido_cobrado_febrero
 from #agentes_filtrados t1 INNER JOIN agentes_extension_docente_historico t2
on t1.numeroControl = t2.NroCOntrol
where t2.mes = 03 and anio = 24
...
NroCOntrol,
	CONVERT(numeric (18) , CAST (dbo.RellenarTextoIzquierda(replace(cast(
			  (convert(decimal (18,2), t2.IMP_521) + (convert(decimal (18,2),t2.IMP_522))) -- total hab.
			- (
					 ISNULL(convert(decimal (18,2), null),'000000000') -- 601
					+ convert(decimal (18,2), t2.IMP_602) 
					+ convert(decimal (18,2), t2.IMP_615) 
					+ convert(decimal (18,2), t2.IMP_616) 					
					+ convert(decimal (18,2), t2.IMP_663) 
					+ convert(decimal (18,2), t2.IMP_664)
					+ convert(decimal (18,2), t2.IMP_665_1) 								
					+ ISNULL(convert(decimal (18,2), null),'000000000') -- 746
				) 
				 -- tot desc. 
			AS decimal(18,2)),'.',''),'0',9) AS VARCHAR(9))) as "liquido actual",  -- CALCULO DEL LIQUIDO,	
			CONVERT(numeric (18,0), t1.total_liquido) as liquido_cobrado_febrero
 from agentes_extension_docente_historico  t1 LEFT JOIN #agentes_filtrados  t2
on t1.NroCOntrol = t2.numeroControl
where t1.mes = 02 and t1.anio = 24
...
and mes = 03 and anio = 24



--SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_abril
--where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
--and NroCOntrol = '38345594'

--SELECT IMP_602, IMP_615, IMP_616, IMP_663, IMP_664, IMP_665_1 from #agentes_filtrados
--where numeroControl = '38345594'
...
WHERE
	CAST (dbo.RellenarTextoIzquierda(replace(cast(
			...
			- (
				 ...
				 -- tot desc. 
			AS decimal(18,2)),'.',''),'0',9) AS VARCHAR(9)) > 0 -- CALCULO DEL LIQUIDO
 

 -- DUPLICADOS -- 
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_abril 
...
-- HAY AGENTES QUE SE LES DESCONTO EL SALDO PERO NO EXISTIAN EN LA LIQUIDACION ORIGINAL
select * from agentes_extdoc_diferencia_saldo_abril t1 
where not exists (
	select * from  #agentes_filtrados t2
	where t1.NroCOntrol = t2.numeroControl
)
and monto_destino_norem_descontado != '0'
...
-- PARA VERIFICAR DIFERENCIAS EN TOTAL HABERES EXCEL - 12.374.137,47
SELECT SUM (convert(decimal (18,2), t1.monto_destino_norem_descontado))
from agentes_extdoc_diferencia_saldo_abril t1

-- TOTAL DESCUENTOS GENERAL -> xxx / $12.349.238.36
SELECT  				
	 SUM (convert(decimal (18,2), t3.monto_destino_norem_descontado))
	 --count(1)
AS Columna 
...
exec [ExtensionDocente.Guardar_En_Historico] '04', '24', 49, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '04', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 49, 0,0,0

...
WHERE 
	...
	AND EXISTS (
		...
		where t2.liq_id in (11,28) -- TODOS LOS LIQID DE MARZO
...
where t2.liq_id in (11,28)  -- TODOS LOS LIQID DE FEBRERO
...
SELECT @liq_id_destino = 37 -- NORMALMENTE ES EL ID DE LA PRIMERA ETAPA DEL MES LIQUIDADO
...
FROM  
	...
	where t3.liq_id in (11,28)  -- TODOS LOS LIQID DE FEBRERO
...
SET
	...
	monto_origen_norem_descontar = (CASE WHEN convert(decimal (18,2), incorrecto.I02) >= convert(decimal (18,2), correcto.I02)
									...
									END)
FROM
	agentes_extdoc_diferencia_0323 correcto
	inner join agentes_extension_docente_historico incorrecto
	on correcto.NroCOntrol = incorrecto.NroCOntrol
	where incorrecto.liq_id in (11,28)  -- TODOS LOS LIQID DE MARZO	

-- A DESCONTAR NO REM -- 5642

...
SELECT 	@mesanioPruebasAge = '11/22'
...
WHERE 
	...
	AND EXISTS (
		...
		where t2.liq_id in (1,4,12,21) -- TODOS LOS LIQID DE NOVIEMBRE
...
FROM  
	...
	where t3.liq_id in (1,4,12,21) -- TODOS LOS LIQID DE NOVIEMBRE
...
SET
	...
	monto_origen_norem_descontar = (CASE WHEN convert(decimal (18,2), incorrecto.I02) >= convert(decimal (18,2), correcto.I02)
									...
									END)
FROM
	agentes_extdoc_diferencia_1122 correcto
	inner join agentes_extension_docente_historico incorrecto
	on correcto.NroCOntrol = incorrecto.NroCOntrol
	where incorrecto.liq_id in (1,4,12,21) -- TODOS LOS LIQID DE NOVIEMBRE
	


-- A DESCONTAR NO REM -- 6714

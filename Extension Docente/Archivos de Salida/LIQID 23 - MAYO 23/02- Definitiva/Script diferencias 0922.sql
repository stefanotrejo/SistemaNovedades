...
SELECT 	@mesanioPruebasAge = '09/22'
...
WHERE 
	...
	AND EXISTS (
		...
		where t2.liq_id in (19, 24) -- TODOS LOS LIQID DE SEPTIEMBRE
...
select * from #agentes_filtrados
where numeroControl in ('38978444',  '38887444')
...
where numeroControl in ('3897844','38877444')
...
SELECT 
	...
	-- AP EMPLEADO ANSES
	SUM (
		convert(decimal (18,2),t1.I03) + convert(decimal (18,2),t1.I08)
		) AS aporteAnsesEmpleado, 
	-- AP PATRONAL ANSES
	SUM (
		 t1.AP_ANSES
	...
	-- TOTAL APORTE ANSES
	SUM (
		convert(decimal (18,2),t1.I03) + convert(decimal (18,2),t1.I08) + t1.AP_ANSES
		) AS ANSES,

	-- AP EMPLEADO IOSEP
	SUM (
		convert(decimal (18,2),t1.I04) + convert(decimal (18,2),t1.I05)
		) AS aporteIosepEmpleado, 
	-- AP PATRONAL IOSEP
	SUM (
		 t1.AP_IOSEP
	...
	-- TOTAL APORTE IOSEP
	SUM (
		convert(decimal (18,2),t1.I04) + convert(decimal (18,2),t1.I05) + t1.AP_IOSEP
		) AS IOSEP,

		-- AP EMPLEADO OSPLAD
	SUM (
		convert(decimal (18,2),t1.I06) + convert(decimal (18,2),t1.I07)
		) AS aporteOspladEmpleado, 
	-- AP PATRONAL OSPLAD
	SUM (
		 t1.AP_OSPLAD
	...
	-- TOTAL APORTE OSPLAD
	SUM (
		convert(decimal (18,2),t1.I06) + convert(decimal (18,2),t1.I07) +  t1.AP_OSPLAD
		) AS OSPLAD		
FROM  
...
SELECT 
	...
	23 --  liq id destino (liquidacion actual)
FROM  
	#agentes_filtrados t1
	INNER JOIN PruebasAge t2	
	ON t1.ageId = t2.NuevoAgeId1
	...
	where t3.liq_id in (19, 24) -- TODOS LOS LIQID DE SEPTIEMBRE
...
SET
	...
	monto_origen_norem_descontar = (CASE WHEN convert(decimal (18,2), incorrecto.I02) >= convert(decimal (18,2), correcto.I02)
									...
									END)
FROM
	agentes_extdoc_diferencia_0922 correcto
	inner join agentes_extension_docente_historico incorrecto
	on correcto.NroCOntrol = incorrecto.NroCOntrol
	where incorrecto.liq_id in (19, 24) -- TODOS LOS LIQID DE SEPTIEMBRE
...
select * from LiquidacionExtensionDocente
where descripcion like '%noviembre%'
...
exec [ExtensionDocente.Guardar_En_Historico] '12', '22', 22, 0, 14

select * from agentes_extension_docente_historico where liq_id = 21
...
EXEC [PagosEventuales.Validar] '31120478', '90104993'

select * from PagosEventuales
where ageNroControl = '90104993'


select * from PruebasAge
where MesAnioLiq = '12/22'
--and NroCOntrol = '38863291'
and NroCOntrol in ( -- SACAR 15.5.23
 '38272063' -- SACAR 15.5.23
,'38548024'-- SACAR 15.5.23
,'38723624'-- SACAR 15.5.23
,'38888314'-- SACAR 15.5.23
,'38388762'-- SACAR 15.5.23
,'38747372'-- SACAR 15.5.23
)

select * from PruebasAge
where MesAnioLiq = '12/22'
and NroCOntrol = '38863291' -- AGREGAR

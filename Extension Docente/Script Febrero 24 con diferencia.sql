...
agentes_extdoc_diferencia_saldo_final_febrero
...
select * from #agentes_filtrados t1 inner join agentes_extdoc_diferencia_saldo_final_febrero t2 
...
UPDATE agentes_extdoc_diferencia_saldo_final_febrero set monto_destino_rem_descontado = 0.00
UPDATE agentes_extdoc_diferencia_saldo_final_febrero set monto_origen_rem_descontar = 0.00


select * from agentes_extdoc_diferencia_saldo_final_febrero
where CONVERT(numeric (18,2),monto_destino_norem_descontado) <= 0


select * from #agentes_filtrados where numeroControl = 38215192
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_saldo_final_febrero t2
	ON T1.numeroControl = t2.NroCOntrol
...
from agentes_extdoc_diferencia_saldo_final_febrero
where
--	 CONVERT(Numeric (18,2),saldo_norem) = 0 -- LES DESCONTÓ TODO
--AND 
CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0

-- NO LOS ENCONTRÓ XXX / 199
select * from agentes_extdoc_diferencia_saldo_final_febrero
where CONVERT(numeric (18,2), monto_destino_norem_descontado) <= 0

-- AGENTES A DESCONTAR ORIGINALMENTE -- XXX / 2585
select * from agentes_extdoc_diferencia_saldo_final_febrero  t1
where 	
...
UPDATE agentes_extdoc_diferencia_saldo_final_febrero
SET fecha_descuento = GETDATE()
...
FROM  
	...
	LEFT JOIN agentes_extdoc_diferencia_saldo_final_febrero t3
	ON t1.numeroControl = t3.NroCOntrol


SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_final_febrero
where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
FROM  
 ...
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_final_febrero 
...
from agentes_extdoc_diferencia_saldo_final_febrero t1
...
exec [ExtensionDocente.Guardar_En_Historico] '02', '24', 45, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '02', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 45, 0,0,0

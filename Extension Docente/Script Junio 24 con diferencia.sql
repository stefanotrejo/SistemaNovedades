...
agentes_extdoc_diferencia_saldo_junio
...
select * from #agentes_filtrados t1 inner join agentes_extdoc_diferencia_saldo_junio t2 
...
--UPDATE agentes_extdoc_diferencia_saldo_junio set monto_destino_rem_descontado = 0.00
--UPDATE agentes_extdoc_diferencia_saldo_junio set monto_origen_rem_descontar = 0.00


--select * from agentes_extdoc_diferencia_saldo_junio
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_saldo_junio t2
	ON T1.numeroControl = t2.NroCOntrol
...
select * from agentes_extdoc_diferencia_saldo_junio t1
inner join #agentes_filtrados t2 
on t1.NroCOntrol = t2.numeroControl -- 339 encuentra controles
...
where not exists (
		select * from agentes_extdoc_diferencia_saldo_junio t3
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 
and t3.NroCOntrol = t2.numeroControl
)


-- TOTAL DESCONTADOS - 332
select * from agentes_extdoc_diferencia_saldo_junio
where CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0 

-- PARCIAL o NULO -  xx / 92
select * from agentes_extdoc_diferencia_saldo_junio
...
from agentes_extdoc_diferencia_saldo_junio
where
...
-- TOTAL DESCUENTOS GENERAL -> xxx / $1.187.574.06
SELECT  				
...
FROM  
	#agentes_filtrados t1		
	LEFT JOIN agentes_extdoc_diferencia_saldo_junio t3
	ON t1.numeroControl = t3.NroCOntrol



--SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_junio
--where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
WHERE
 ...
 -- DUPLICADOS -- 
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_junio 
...
from agentes_extdoc_diferencia_saldo_junio t1
...
exec [ExtensionDocente.Guardar_En_Historico] '06', '24', 56, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '06', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 54, 0,0,0

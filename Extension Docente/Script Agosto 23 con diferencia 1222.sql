...
SELECT 	@mesanioPruebasAge = '08/23'
...
UPDATE agentes_extdoc_diferencia_1222 set monto_destino_rem_descontado = null
UPDATE agentes_extdoc_diferencia_1222 set monto_destino_norem_descontado = null
...
FROM
	...
	INNER JOIN agentes_extdoc_diferencia_1222 t2
...
-- NO LOS ENCONTRÓ 355 / 281
select * from agentes_extdoc_diferencia_1222
where saldo_norem is null
AND CONVERT(numeric (18,2),monto_origen_norem_descontar) > 0 


-- AGENTES A LOS QUE NO SE LE PODRA COBRAR POR DISTINTOS FILTROS -- 512
select * from agentes_extdoc_diferencia_1222  t1
...
select t2.IMP_522, t1.monto_origen_norem_descontar, t1.monto_destino_norem_descontado from agentes_extdoc_diferencia_1222 t1 
...
--	#agentes_filtrados t1
--INNER JOIN 
--	agentes_extdoc_diferencia_1222 t2
--ON  T1.numeroControl = t2.NroCOntrol
...
FROM  
	...
	LEFT JOIN agentes_extdoc_diferencia_1222 t3
...
from agentes_extdoc_diferencia_1222 t1
where id not in (3018) -- ID DE LOS AGENTES DUPLICADOS
	-- ids de controles pagados 2 veces en liq 22 (ya habian cobrado en liq 3)}
	-- esto fue causado por un pedido de pago de una lista de nro de controles 
...
select SUM (CONVERT(numeric (18,2), monto_destino_norem_descontado)) from agentes_extdoc_diferencia_1222


exec [ExtensionDocente.ArchivoRectificativaV2] 32, 0,0,0

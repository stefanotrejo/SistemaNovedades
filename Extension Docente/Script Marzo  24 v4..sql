...
select t1.numeroControl ,CAST (dbo.RellenarTextoIzquierda(replace(cast(
			  (convert(decimal (18,2), t1.IMP_521) + (convert(decimal (18,2),t1.IMP_522))) -- total hab.
			- (
					 ISNULL(convert(decimal (18,2), 0),'000000000') -- 601
					+ convert(decimal (18,2), t1.IMP_602) 
				 ...
				 -- tot desc. 
			AS decimal(18,2)),'.',''),'0',9) AS VARCHAR(9)),
			t3.control
			,t3.a_favor
			, t4.I10
...
from 
	...
	INNER JOIN PruebasAge t2	
	ON t1.ageId = t2.NuevoAgeId1
	INNER JOIN marzo24_mario t3
	on t3.control = t1.numeroControl
	LEFT JOIN agentes_extension_docente_historico t4
	on t1.numeroControl = t4.NroCOntrol
WHERE
	CAST (dbo.RellenarTextoIzquierda(replace(cast(
			  (convert(decimal (18,2), t1.IMP_521) + (convert(decimal (18,2),t1.IMP_522))) -- total hab.
			- (
...
AND t4.mes = 01
and t4.anio = 24
order by CONVERT(numeric (18),t4.I10)

select * from marzo24_mario
where nrocontrol = '38657022'
...
select * from agentes_extension_docente_historico
where mes = 02 and anio = 24 and liq_id = 50
...
exec [ExtensionDocente.Guardar_En_Historico] '03', '24', 51, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '03', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 51, 0,0,0
...
select NroCOntrol, total_liquido from agentes_extension_docente_historico
WHERE liq_id = 50
AND NroCOntrol = '38001543'
...
where CONVERT(numeric (18,2),I01) <= 0
and CONVERT(numeric (18,2),I02) > 0
and liq_id = 50
and 
...
where 
 ...
 liq_id = 43
and NroCOntrol = '38729515'

select * from LiquidacionExtensionDocente
wh

select * from #agentes_filtrados
WHERE numeroControl = '38920912' or numeroControl = '38921324'

select * from agentes_extdoc_diferencia_saldo_abril
where  NroCOntrol = '38920912' or NroCOntrol = '38921324'

select * from agentes_extension_docente_historico
where liq_id = 43l
and NroCOntrol = '59152145'

and (NroCOntrol = '38920912' or NroCOntrol = '38921324')
...
select * from #agentes_filtrados t1 inner join agentes_extension_docente_historico t2
on t1.numeroControl = t2.NroCOntrol
 
where mes = 01 and anio = 24
and CONVERT(DECIMAL (18,2),t2.I10) > 0

select * from agentes_extension_docente_historico
where liq_id = 50
and NroCOntrol = '38141791'

and NroCOntrol = '38918465'

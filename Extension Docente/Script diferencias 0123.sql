...
WHERE 
	...
	AND EXISTS (
		...
		where t2.liq_id in (7,18) -- TODOS LOS LIQID DE ENERO
...
FROM  
	...
	where t3.liq_id in (7,18)  -- TODOS LOS LIQID DE ENERO
...
SET
	...
	monto_origen_norem_descontar = (CASE WHEN convert(decimal (18,2), incorrecto.I02) >= convert(decimal (18,2), correcto.I02)
									...
									END)
FROM
	agentes_extdoc_diferencia_0123 correcto
	inner join agentes_extension_docente_historico incorrecto
	on correcto.NroCOntrol = incorrecto.NroCOntrol
	where incorrecto.liq_id in (7,18)  -- TODOS LOS LIQID DE ENERO
	


-- A DESCONTAR NO REM -- 5643

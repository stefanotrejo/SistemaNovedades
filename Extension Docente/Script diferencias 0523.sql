...
CAMBIAR:
- SELECT 	@mesanioPruebasAge = 
- "liq_id in"
	lineas: 211, 
- NOMBRE DE TABLA DE DIFERENCIAS
- LIQ ID DESTINO PARA LA TABLA DE DIFERENCIAS
agentes_extdoc_diferencia_0523
*/
...
WHERE 
	...
	AND EXISTS (
		...
		where t2.liq_id in (17) -- TODOS LOS LIQID DE MARZO
...
SELECT @liq_id_destino = 39 -- NORMALMENTE ES EL ID DE LA PRIMERA ETAPA DEL MES LIQUIDADO
...
FROM  
	...
	where t3.liq_id in (17)  -- TODOS LOS LIQID DE ABRIL
...
SET
	...
	monto_origen_norem_descontar = (CASE WHEN convert(decimal (18,2), incorrecto.I02) >= convert(decimal (18,2), correcto.I02)
									...
									END)
FROM
	agentes_extdoc_diferencia_0523 correcto
	inner join agentes_extension_docente_historico incorrecto
	on correcto.NroCOntrol = incorrecto.NroCOntrol
	where incorrecto.liq_id in (17)  -- TODOS LOS LIQID DE ABRIL	

-- A DESCONTAR NO REM -- 5603
SELECT * from agentes_extdoc_diferencia_0523 	
...
select  NroCOntrol from agentes_extdoc_diferencia_0523 

...
UPDATE agentes_extdoc_diferencia_saldo_final set monto_destino_rem_descontado = 0.00
UPDATE agentes_extdoc_diferencia_saldo_final set monto_origen_rem_descontar = 0.00
...
from agentes_extdoc_diferencia_saldo_final
where
--	 CONVERT(Numeric (18,2),saldo_norem) = 0 -- LES DESCONTÓ TODO
--AND 
CONVERT(numeric (18,2),monto_destino_norem_descontado) > 0

-- NO LOS ENCONTRÓ XXX / 135
select * from agentes_extdoc_diferencia_saldo_final
where saldo_norem is null
AND CONVERT(numeric (18,2),monto_origen_norem_descontar) > 0 

-- AGENTES A DESCONTAR ORIGINALMENTE -- 5603 / 3492
select * from agentes_extdoc_diferencia_saldo_final  t1
where 	
...
-- TOTAL DESCUENTOS GENERAL -> $20.125.356.94 / $4.421.531.46
SELECT  				
...
FROM  
	...
	LEFT JOIN agentes_extdoc_diferencia_saldo_final t3
	ON t1.numeroControl = t3.NroCOntrol



SELECT nrocontrol, monto_destino_norem_descontado, monto_destino_rem_descontado, monto_origen_norem_descontar FROM agentes_extdoc_diferencia_saldo_final
where CONVERT(numeric (18,2) ,monto_destino_norem_descontado) > 0
...
FROM  
 ...
 select  NroCOntrol from agentes_extdoc_diferencia_saldo_final 
...
from agentes_extdoc_diferencia_saldo_final t1
------------------------------------------------------
...
exec [ExtensionDocente.Guardar_En_Historico] '01', '24', 43, 0, 0

exec [ExtensionDocente.Archivo_Ministerio] '01', '24'

exec [ExtensionDocente.ArchivoRectificativaV2] 43, 0,0,0
